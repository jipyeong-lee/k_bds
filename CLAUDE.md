...
| **포트 노출이 없다** | 플랫폼 API 에 ingress·포트포워딩 기능이 없어 노드에서 띄운 vLLM 서버(8000)는 **job 안에서만 보인다**. 배치 추론은 되고(job 안에서 호출 → 결과를 `$ORCH_HOME` 파일로 → 파일 API 로 회수) 상시 서비스는 별도 인프라가 필요하다. 실측 → [`b200/demo/vllm_test.sh`](b200/demo/vllm_test.sh) |
| **`archive` 로 tar.gz 내보내기** | `GET /me/data/archive?path=<디렉터리>` 가 디렉터리를 tar.gz 로 준다. **safetensors 500 버그를 애초에 우회하는 경로다** — 85G 이관 때 이걸 알았으면 tar 스테이징을 직접 만들 필요가 없었다. 대용량 회수는 이쪽을 먼저 시도할 것 |
