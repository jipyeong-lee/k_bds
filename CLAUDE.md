...
| | 내용 |
|---|---|
| job 수명 | `timeout_sec` 이 그대로 상한. 3600 을 주면 3600 초에 `killed`, **7200·14400 모두 생성 수락**(60분 고정 아님. 14400 은 2026-08-18 실측 — 생성 90초 뒤 swift 진입 확인). **길게 잡을수록 이득**: job 이 죽으면 마지막 체크포인트 이후 진행분(`save_steps=16` 기준 평균 ~4 step)이 통째로 버려져 **교체 횟수가 곧 손실**이다. job 내부는 199 s/step 인데 7200 실효는 **226.5**(실측), 14400 이면 산술 **212** → 하루 381 → 408 step |
