...
> 다만 이는 **포화의 증거가 아니라 분해능 부족**이다 — n=300 의 검출 하한은 **8.0pp** 로, 200 step 당 1~2pp 의
> 완만한 상승은 배제되지 않는다(그 기울기면 잔여 1,653 step 에서 **+8~16pp**).
> → **n=1,200 + 짝지음으로 하한을 2.8pp 로 낮추고 step 1200 에서 판정**한다. 그때까지 학습은 계속한다.
> 목표 도메인인 **의료(pmcvqa)는 상승 신호 없음**(0.57→0.56→0.57→0.53, 단 소스별 n=100 ±9.8pp) — 반면 math 는 0.40→0.48~0.50 으로 뚜렷.
