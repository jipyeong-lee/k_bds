...
**다음 임계경로**: **평가 검정력 보강**(n=300→**전량 1,772** + 짝지음, 검출 하한 8.0pp→2.3pp, ≈2~3 노드시간) → **step 1200 에서 사전기준 판정**(계속/중단) → **Stage-3 본실행**(계획서 핵심 산출물, 미시작 — 의료를 올리도록 설계된 유일한 단계).
...
> → **홀드아웃 전량(1,772) + 짝지음으로 하한을 2.3pp 로 낮추고 step 1200 에서 판정**한다. 그때까지 학습은 계속한다.
