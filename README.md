...
>
> 🔍 **소스별로 끊어 보면 정확도는 정지해 있지 않다.** `completions.jsonl` 의 25,632 개 문항별 채점 결과 —
> **deepvision(데이터 67%)은 +0.54pp/100step 으로 유의하게 상승**(t=3.02, 잔여 외삽 +8.3pp [95% +2.9~+13.7]),
> 수학(mmk12)·의료(pmcvqa)는 추세 미검출. 홀드아웃 n=300 은 3 소스 균등이라 이 이득이 1/3 로 희석돼 **+2.8pp** 가 되고,
> 이는 검출 하한 8.0pp 아래다 — §1 의 "폭 1.0pp" 가 이렇게 설명된다.
> → [보고서 §6-c](docs/stage2_run73924_progress.md) · 재현 `python3 scripts/train_source_trend.py`
...
> 다만 이는 **포화의 증거가 아니라 분해능 부족**이다 — n=300 의 검출 하한은 **8.0pp** 다.
> 이제는 가정이 아니다: 학습 로그의 25,632 문항이 **deepvision +0.54pp/100step 상승**(t=3.02)을 실측했고,
> 3 소스 균등 층화에서 이는 합성 **+2.8pp** 로 희석돼 하한 아래로 떨어진다(→ [§6-c](docs/stage2_run73924_progress.md)).
...
>
> 🔀 **두 측정이 어느 소스가 오르는지에서 엇갈린다** — 홀드아웃은 math 상승(0.40→0.48~0.50)·의료 하락(0.57→0.53)으로 읽히는데,
> 학습 로그는 **deepvision 만 상승**하고 math·의료는 추세 미검출이다. 표본은 학습 로그가 압도적이지만(3,972·4,496 vs 각 100)
> 홀드아웃은 temperature=0.0 이고 학습은 0.9 라 같은 것을 재는 것도 아니다.
> **이 불일치를 남겨 두는 것 자체가 n=1,200 재측정이 필요한 이유다** — 소스별 n 이 100 → 400 이 되면 갈린다.
