...
**지금 위치**: **Stage-2 풀확장 본실행 중** (k252a02 이관 완료, MAX_STEPS=2,337 체인 job **73924~73927**) — **1,038/2,337 step (44%)** 통과.
**다음 임계경로**: **step 1200 사전기준 판정**(오늘 22:30경 도달, `watch_and_eval.sh` 가 감시·스냅샷·제출 자동화) → **Stage-3 본실행**(계획서 핵심 산출물, 미시작 — 의료를 올리도록 설계된 유일한 단계).

**측정 분해능 문제는 해결됐다.** 홀드아웃 **전량 1,772 + 짝지음(McNemar)** 으로 검출 하한이 8.0pp → **1.7~2.3pp** 가 됐고,
init·400·500·600·700 다섯 지점을 **같은 문항 위에서** 다시 쟀다. 결과는 셋으로 갈린다.

- ✅ **RL 순효과는 확정** — init → step 400 **+5.70pp, p<0.0001**. n=300 시절 ±9.8pp 노이즈에 묻혀 있던 것이 드러났다.
- ❓ **step 400 이후는 여전히 미검출** — 400 → 700(300 step) **+1.35pp, p=0.18**. 다만 이제는 상한이 붙는다:
  95% 구간이 **[−0.6, +3.3]pp** 라 "큰 상승은 없다"까지는 말할 수 있고, +3pp 는 아직 배제되지 않는다.
- 🚨 **의료(pmcvqa)는 네 지점 모두에서 움직이지 않았다** — init 대비 +0.25 / −0.75 / −0.25 / +1.25pp, 전부 p>0.5.
  n=400 으로 재측정해도 그대로다. **RL 이 의료에 순효과를 못 내고 있다**는 것이 Stage-3 설계의 전제가 된다.
...
| **② 범용 RLVR** | 🚀 **본실행 중 (44%)** · ✅RL 효과 확정 · ❓추가 상승 미검출 | 확장셋 **74,787**(일반53/math20/의료26), init=v3·GDPO. 전량 1,772 짝지음: **init→400 +5.70pp(p<0.0001)**, **400→700 +1.35pp(p=0.18)**, 의료 무변화 → [중간점검](docs/stage2_run73924_progress.md) · [실험](docs/stage2_experiments.md) · [데이터](docs/stage2_data.md) · [실행현황](#stage-2-본실행-현황) |
...
**2026-08-04 09:20 KST 기준 · job 73925 (`gpu-8-002`) · 로그 `logs/grpo_adv_73925.log`**

| 항목 | 값 | 판정 |
|---|---|---|
| 진행 | **1,038 / 2,337 step (44.4%)** = 0.111 epoch | 정상 |
| 체인 | 73924 종료(TimeLimit, step 787) → **73925 가 checkpoint-750 에서 재개**(25h 경과 / 한도 70h) · 73926/27 대기 | ✅ 인계 정상 |
| 속도 | **~296 s/it** (벽시계, `train_speed`) — 73924 의 318.5 보다 개선 | ⚠️ 오버헤드 |
| 잔여 추정 | 완주 ≈ **08-08~09** · **판정 시점(step 1200) 도달 = 08-04 22:30경** | — |
| 안정성 | OOM · CUDA error · Traceback **0건** | ✅ 무결 |

> 📌 **step 1200 판정은 자동화돼 있다.** `save_total_limit=5` 라 체크포인트는 저장 후 ~20h 뒤 롤링 삭제되는데,
> 도달 시각이 밤 22:30 이라 사람이 그 창에 붙어 있어야 했다. `scripts/watch_and_eval.sh` 가 로그인 노드에서
> 5분 간격으로 감시하다가 **나타나는 즉시 스냅샷을 뜨고 평가를 제출**한다. 스냅샷이 제출보다 먼저이므로
> sbatch 가 실패해도 체크포인트는 이미 안전하다.
...
**중간 홀드아웃 추세 (job 74187·74137·74188·74189·74190, 전량 n=1,772)** — 확장 홀드아웃 **전량**, greedy, 다섯 모델 **동일 문항**
...
| format / 평균 길이 | 0.947 / 1,812 | 0.892 / 2,124 | 0.887 / 2,103 | 0.887 / 2,126 | 0.903 / 2,031 |

**짝지음(McNemar) 판정** — 같은 문항을 두 모델이 푼 것이므로 짝지음이 맞다. 하한 ±9.8pp → **±1.7~2.3pp**
...
> ✅ **RL 은 듣는다**: init → 400 **+5.70pp(p<0.0001)**. n=300 시절 "+3.67pp, 판정불가"로 남았던 것이 전량 측정에서 확정됐다.
> ❓ **그러나 그 이득은 step 400 까지에 거의 다 실렸다**: 400 → 700 은 300 step 을 더 돌고 **+1.35pp(p=0.18)**.
> 이건 "차이 없음"이 아니라 **95% 구간 [−0.6, +3.3]pp** 다 — 큰 상승은 없지만 +3pp 는 배제되지 않는다.
> **400 vs 1200 의 하한이 ±1.9pp** 이므로, 실제 Δ 가 +3pp 라면 **step 1200 측정에서 검출된다**. 판정은 그때다.
>
> 🚨 **의료(pmcvqa)는 네 지점 전부 미검출** — +0.25 / −0.75 / −0.25 / +1.25pp, 모두 p>0.5. n=100(±9.8pp) 시절엔
> "노이즈일 수 있다"였지만 **n=400(±3.5pp)에서도 그대로**다. 사전등록 중단선(−3pp)에는 안 걸리나,
> **RL 이 목표 도메인을 올리지 못하고 있다**는 사실 자체가 Stage-3(의료 RaR)의 존재 이유를 강화한다.
>
> ⚠️ **부수 비용 — format 이 6pp 깎였다**: 0.947 → 0.887~0.903, 동시에 평균 길이 1,812 → ~2,100.
> [길이 구성효과](docs/stage2_run73924_progress.md)(장문 응답 deepvision 15.1% vs 비장문 44.2%)와 같은 방향이다.
> 전체 정확도가 오르는 와중에도 이 손실은 누적되고 있다.
>
