...
mkdir -p "$R/parts"; rm -f "$R"/parts/*.part
echo "== split large tars ($CHUNK) =="
for t in model.tar data.tar pmcvqa_data.tar; do
  [ -f "$R/$t" ] && split -b "$CHUNK" -d --additional-suffix=.part "$R/$t" "$R/parts/${t}."
done
ls -lh "$R"/parts/*.part 2>/dev/null | awk '{print "  "$5, $9}'

echo "== upload code (whole) =="; [ -f "$R/code.tar.gz" ] && up "$R/code.tar.gz" uploads
echo "== upload parts =="
for f in "$R"/parts/*.part; do up "$f" uploads/parts || { sleep 2; up "$f" uploads/parts || echo "  FAILED $(basename "$f")"; }; done
