...
- [ ] **Stage-2 재시작 설계** ← **다음 임계경로**. ① `overlong_filter` 검증·해제 ② KL `beta` 0.04 → 상향 ③ 형식 감시 조기중단(`FormatThink < 0.85` 지속 시) ④ 재시작 지점 선택(init / ~400 / 850). ⑤ `max_grad_norm` 은 1.0 이면 무의미(실측 최대 0.17) → 하향 검토
