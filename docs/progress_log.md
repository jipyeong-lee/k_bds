...
- **08-03** — **체인 인계 확인 + 학습 로그 소스별 추세 측정**. ① 73924 가 TimeLimit(07:40, step 787)로 종료되고 **73925 가 checkpoint-750 에서 정상 재개**(현재 step 762/2,337). 재개마다 출력 디렉터리가 새로 생긴다는 것 확인 — `v0-20260731-094532` → **`v1-20260803-074645`**, 문서의 v0 고정경로를 `ls -dt … | head -1` 로 교체. TimeLimit 컷은 마지막 저장 이후 **37 step 유실**(save_steps=50 구조상 정상). 실측 속도 **145 s/it**(길이 감소로 400~699 의 168 s/it 보다 빨라짐) → **step 1200 까지 약 18h**, 종전 문서의 "1.9일"은 누적평균 322 s/it 기준이라 과대추정이었음. ② **`completions.jsonl` 에 문항별 채점 결과가 이미 있었다** — 홀드아웃 n=300 의 **85배**인 25,632 완성문. 질문 텍스트로 학습셋과 조인해 소스 100% 매칭(`scripts/train_source_trend.py` 신설, 호스트 python3 동작). **deepvision(67%) +0.537pp/100step, t=3.02 유의**(잔여 1,550 step 외삽 **+8.3pp** [95% +2.9~+13.7]) / mmk12 −0.244 (t=−0.57) / pmcvqa −0.208 (t=−0.65) 둘 다 미검출. ③ **이것이 §1 의 "폭 1.0pp"를 정량적으로 설명한다** — 홀드아웃은 3 소스 균등 층화라 deepvision 단독 +8pp 는 합성 **+2.8pp** 로 희석되고, 이는 검출 하한 8.0pp 의 1/3 이다. 즉 포화가 아니라 **희석 + 분해능 부족**. ④ 교란요인 배제: 학습셋이 소스순 정렬(deepvision 0~53%)이라 셔플이 없으면 2,337 step 내내 deepvision 만 학습될 뻔했으나 `dataset_shuffle=True`·`train_dataloader_shuffle=True` 확인 / 최종 epoch 0.25 라 문항 재등장 없음(**fresh-sample 측정**) / 절단 보정 후에도 44.69→47.06% 상승 유지. ⑤ **200~500 구간의 길이 인플레이션은 지나갔다**(길이 1,127→1,660→1,163, 절단 4.9→9.8→3.7%, 형식 0.947→0.899→0.960) — **step 400/500/600 홀드아웃 평가는 하필 이 저점에서 찍힌 것**. ⑥ **미해결 불일치 기록**: 홀드아웃은 math 상승·의료 하락으로 읽히는데 학습 로그는 deepvision 만 상승이다. 표본은 학습 로그가 압도적이나(3,972·4,496 vs 각 100) temperature 가 다르다(0.0 vs 0.9). **n=1,200 재측정에서 갈린다** — 이 불일치가 재측정 필요의 추가 근거. ⑦ 판정축을 **deepvision 층**으로 고정(§7-2b) — 유일하게 추세가 검출되고 홀드아웃 n 도 400 으로 가장 크다.
...
- [x] **학습 로그 소스별 추세 측정**(`scripts/train_source_trend.py`, 완성문 25,632) → **deepvision +0.54pp/100step 유의**(t=3.02), math·의료 미검출. 홀드아웃이 왜 평평했는지 설명됨
