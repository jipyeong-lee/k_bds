...
- [ ] **Stage-2 풀확장 본실행**(진행중) → 2,337 step 체인 job **73924~73927**(=0.25 epoch, 종전 "1 epoch" 표기는 오류 — [정정](stage2_run73924_progress.md#3-epoch-커버리지--계획-전제가-4배-틀렸다)). **계속 진행 중** — 중단 판단은 step 1200 재평가까지 보류
...
- [ ] **평가 검정력 보강** — `eval_compare.py` per-item 점수 저장 + `EVAL_N=all`(전량 1,772) → 짝지음(McNemar) 검정. 하한 8.0pp → 2.3pp. **계속/중단 판정의 전제조건**
- [ ] **step 1200 판정** — step 400 대비 800 step 간격. 사전기준: ≥+3pp&p<0.05 완주 / −1~+3pp 조기중단 / <−1pp 즉시중단. [중간점검 §7](stage2_run73924_progress.md)
