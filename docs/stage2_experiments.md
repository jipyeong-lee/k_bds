...
- 🚀 **본실행 진행중(2026-07-28~)**: 배선 스모크 완주(1GPU job 72844 · 8GPU job 72832, 5/5 step, `frac_reward_zero_std` 0) → **2,337 step 체인 제출**(`scripts/launch_stage2_expanded_epoch.sh`, afterany 4잡, ~209h). 재제출 후 현재 체인은 **job 73924~73927**(73312~73315 는 구 체인).
  ...
  - 🚨 **중간 홀드아웃 추세(job 74060·74062·74063, n=300 층화 동일 슬라이스)**: base 0.2500 → init **0.4533** → step400 **0.4900** → step500 **0.4800** → step600 **0.4867**. **step 400 이후 판별 불가** — 400~600 폭 1.0pp 는 n=300 의 검출 하한(8.0pp)보다 훨씬 작아 구분되지 않는다. ⚠️ 이는 **포화의 증거가 아니다**: 200 step 당 1~2pp 의 완만한 상승이 배제되지 않으며 그 기울기면 잔여 1,653 step 에서 +8~16pp. 의료(pmcvqa)는 상승 신호 없음(0.57→0.56→0.57→0.53, 소스별 n=100 ±9.8pp), math 는 0.40→0.48~0.50 으로 뚜렷. base→init 은 +20.33pp p<0.001 로 유의 → 평가 설계는 유효. **→ 중단 판단 보류. n=1,200 + 짝지음(McNemar)으로 하한을 2.8pp 로 낮춰 step 1200 에서 사전기준 판정**(≈2~3 노드시간 = 결정 대상 1,184 의 0.2%). → [`stage2_run73924_progress.md`](stage2_run73924_progress.md) §6 · [발표용 요약](stage2_overview_for_slides.md)
