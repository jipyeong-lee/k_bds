...
- 🚀 **본실행 진행중(2026-07-28~)**: 배선 스모크 완주(1GPU job 72844 · 8GPU job 72832, 5/5 step, `frac_reward_zero_std` 0) → **2,337 step 체인 제출**(`scripts/launch_stage2_expanded_epoch.sh`, afterany 4잡, ~209h). 재제출 후 현재 체인은 **job 73924~73927**(73312~73315 는 구 체인).
  ...
  - 🚨 **중간 홀드아웃 추세(job 74060·74062·74063, n=300 층화 동일 슬라이스)**: base 0.2500 → init **0.4533** → step400 **0.4900** → step500 **0.4800** → step600 **0.4867**. **step 400 에서 포화** — 400~600 폭이 1.0pp 로 오차(±5.7pp)보다 작아 구분 불가, 200 step 에 −0.33pp(143 노드시간). 의료(pmcvqa)는 전 구간 상승 없음(0.57→0.56→0.57→0.53), math 는 0.40→0.48~0.50 으로 뚜렷. base→init 은 +20.33pp p<0.001 로 유의 → 평가 설계는 유효. **→ 조기중단 → Stage-3 전환이 근거에 부합.** → [`stage2_run73924_progress.md`](stage2_run73924_progress.md) §6 · [발표용 요약](stage2_overview_for_slides.md)
