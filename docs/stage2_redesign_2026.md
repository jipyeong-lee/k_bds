...
### 🔴 발견 ⑥ — 가중치가 아니라 **advantage 분산 점유**를 봐야 한다

GRPO 는 그룹 평균을 뺀다. 따라서 보상항의 실제 영향력은 **가중치가 아니라 `가중치 × 그룹 내 SD`** 다.
(그룹 = 프롬프트 1개 × 4생성. 로그에서 연속 4칸이 한 그룹임을 확인)
...
**문제는 퇴화 구간에서 일어나는 통제권 이양이다.** 정확도가 그룹 내에서 균일해지면
(4생성 전부 같은 점수) 정확도의 advantage 기여가 **정확히 0** 이 되고, 부수적이어야 할
형식·길이가 **목적함수 전체**가 된다. 개시 시점에 그런 그룹이 **13.8% → 54.2%** 로 4배가 되고,
**그때 정책을 이끄는 건 길이다.**
...
[사후분석](stage2_run73924_postmortem.md)과 [README](../README.md)는 `num_generations=4` 부족 가설을
**`frac_reward_zero_std` 평균 0.0106** 을 근거로 기각했다. 그런데 그 지표는
**총보상**의 그룹 내 SD 가 0 인 비율이다. 형식·길이가 동점을 깨주므로 **정확도 신호가 죽어도
총보상은 여전히 varied 로 보인다.**
...
**둘은 다른 것을 재고 있었다.** 총보상은 끝까지 varied 였지만 **정확도는 그렇지 않았다.**
`num_generations=4` 기각의 근거를 정정한다 — **기각은 유지하되**(붕괴전 13.8% 는 정상 범위),
**근거는 `frac_reward_zero_std` 가 아니라 정확도 균일도여야 한다.**
...
> ⚠️ **아직 검증된 규칙이 아니다.** [§1-5-1](stage2_run73924_postmortem.md) 에서 KL 이
> 오프라인으로는 선행하는데 온라인 규칙으로는 1 step 밖에 못 벌었던 전례가 있다.
> **감시자에 넣기 전에 같은 오경보 스윕을 통과해야 한다.** 다만 기저(751-850 에서 10%)가
> 안정적이라 KL 보다 조건이 낫다.
...
> E2 만 올리면 롤아웃 비용이 그 전문가에서만 2배가 된다(≈ +733 GPU-h).
> **도메인 분리를 했기 때문에 이런 국소 조정이 가능해졌다.**
