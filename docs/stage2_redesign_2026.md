...
| 리포트 | 보상 서술 |
|---|---|
| [**Kimi K3**](https://arxiv.org/abs/2607.24653) | 🟢 **상세** — 검증가능 답 · 루브릭 판정(Agentic GRM, 필수 프로토콜: 결과 읽기→루브릭 생성→채점→기록) · **장황도 제어**(σ·ℓ₀ 초과 시 이진비교 자동 패) · **토큰 예산 초과 시 보상 −1 덮어쓰기** · 커널 성능 보상 |
| [**HyperCLOVA X 32B Think**](https://arxiv.org/html/2601.03286v1) | 🟢 **상세** — 가중합 = 환경보상(규칙 검증 + LLM judge) + **format** + **언어 일관성** + **반복 억제** |
| [**GLM-4.1V/4.5V**](https://arxiv.org/pdf/2507.01006) | 🟢 **수치** — `r_format ∈ {0, 0.5}` · `r_accuracy ∈ {0, 1}` |
| [Qwen3-VL](https://arxiv.org/abs/2511.21631) | 🟡 항목만 — 지시따르기(내용·형식·길이·구조화 출력) + 선호정렬 · **code-switching 페널티** |
| [K-EXAONE 2.0](https://arxiv.org/abs/2608.04505) | 🟡 항목만 — 수학 답 검증(증명은 judge) · 코드 실행 검증 · 인스턴스별 루브릭 · 길이 통제. **형식은 보상이 아니라 데이터 구성으로** |
| [EXAONE 4.5](https://arxiv.org/abs/2604.08644) | 🔴 미상세 — "이전 모델과 같은 보상"·"태스크별 보상 함수" |
| [DeepSeek-V4](https://arxiv.org/html/2606.19348v1) | 🔴 미상세 — "성공 기준에 맞춘 보상 모델" |

> **절반은 상세하고 절반은 얇다.** 그리고 **우리가 구조를 따라 하려는 DeepSeek-V4 가 가장 얇은 쪽**이다.
> 보상 설계 근거는 **Kimi K3 · HyperCLOVA X · GLM-4.1V** 에서 가져와야 한다.
...
soft_overlong   0.2   min( −(len − 4096) / 2048 , 0 )      ← rewards/orm.py:216
```

### 🔴 발견 ① — `soft_overlong` 은 정상 구간에서 죽어 있다

`soft_max_length=6144` · `soft_cache_length=2048` → **4,096 토큰 아래는 항상 0** 이다.
붕괴 전 평균 길이가 **1,376** 인데 임계가 4,096이다. **정상 작동 구간 전체에서 신호가 없다.**
...
**개시 시점에 반응하지 않는다.** 사후분석의 "길이는 원인이 아니라 결과"와 정확히 부합한다.
이건 **길이 제어기가 아니라 절단 방지 장치**다 — [Kimi K3](https://arxiv.org/abs/2607.24653) 의
문제별 적응 예산 `b₀(x)` 와 근본적으로 다르다.

**조치**: 도메인 분리로 **`soft_max_length` 를 도메인별로** 잡는다.
의료 pmcvqa 는 답이 letter 하나인데 6,144 는 과하다.

### ❌ 발견 ② — `repetition` 보상은 추가하지 말 것 (실측 기각)

ms-swift 에 내장돼 있고(`rewards/orm.py:176`, 단어 3-gram, `max_penalty=-1.0`)
HyperCLOVA X 도 반복 억제를 쓴다. 우리 붕괴가 `AAAA…` 4만 자 · `<think>` 1,497회였으니
당연히 후보다. **그런데 실제 롤아웃에 적용해보니 셋 다 틀렸다.**
...
1. **정상 구간에 이미 −0.11 이 상시 부과된다** — 건강한 추론에도 3-gram 반복이 있다. 순수 잡음 드래그다.
2. **최악의 퇴화를 놓친다** — 퇴화(−0.153)가 확산(−0.315)보다 **약하다**.
   `zipngram` 이 `text.lower().split()` 이라 공백 없는 `AAAA…` 는 **단어 1개**로 세어지고
   `len(completion.split()) < ngram_size` 로 걸러져 **0.0** 을 받는다.
3. 확산 구간에서만 반응 — 이미 늦다.
...
한글 code-switching 은 **사실상 0**. 한자는 5.45% 기저가 있으나 **mmk12 가 중국어 원 출처**라
정당한 내용일 가능성이 높다. 페널티를 걸면 **정상 콘텐츠를 벌준다.**

### ⚠️ 발견 ④ — 형식 가중치 0.2 는 참조 대비 낮지만, 올리지 않는다

[GLM-4.1V](https://arxiv.org/pdf/2507.01006) 는 형식이 정확도의 **50%**(0.5 대 1.0), 우리는 **20%**(0.2 대 1.0)다.

> **그런데 올리는 게 무조건 옳지 않다.** 형식 가중치는 **두 가지를 동시에** 조절한다 —
> ① 형식을 지킬 유인 ② **형식을 잃었을 때의 낙폭**. 올리면 ①이 강해지지만 ②도 깊어져
> 되먹임 고리가 세진다. 우리가 겪은 게 정확히 ②다.
>
> **단계형 보상(§3-B)이 이 결합을 끊는다** — 부분점수라 낙폭이 가중치에 선형 비례하지 않는다.
> **0.2 를 유지하고 단계형부터 넣는다.** 재실행에서 `rewards/FormatThink/mean` 추세를 보고 조정한다.

### 발견 ⑤ — `accuracy_mix` 가 이진인 것은 지금 문제가 아니다

Kimi K3 는 루브릭 부분점수를 쓴다. 우리 정확도는 0|1 뿐이다.
객관식·수치 답에서는 자연스럽고, 부분점수가 필요해지는 지점은 **medix 단답 서술**인데
그건 Stage-3 의 `clinical_judge`(루브릭)가 담당한다.
→ [E3-b](deepseek_v4_pipeline_adoption.md) 에서 해결된다. **지금 바꿀 것 없다.**
