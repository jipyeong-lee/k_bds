...
| **2** | **step 1200 에서 판정** (step 400 대비 800 step 간격) | 간격이 커야 완만한 기울기가 누적돼 검출 하한을 넘는다. **§6-c 가 실패했으므로 이 측정이 유일한 근거다** — 학습 로그로는 앞으로의 기울기가 판별되지 않는다(CI ±14pp). 도달까지 순수 실행 **약 18시간**(2026-08-03 09:00 기준 step 762, 실측 145 s/it) | **높음** |
