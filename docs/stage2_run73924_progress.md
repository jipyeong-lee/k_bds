...
| **2** | **step 1200 에서 판정** (step 400 대비 800 step 간격) | 간격이 커야 완만한 기울기가 누적돼 검출 하한을 넘는다. 400→1200 은 관측 궤적(+1pp/200step)이 참이면 +4pp > 2.8pp → **판별 가능**. 도달까지 순수 실행 **약 1.9일**(큐 대기 별도) | **높음** |
| **2b** | **사전 중단기준**(사후 해석 방지 — 지금 못 박는다) | ⓐ step1200 − step400 ≥ **+3pp** & 짝지음 p<0.05 → **완주**<br>ⓑ **−1pp ~ +3pp** → **조기중단 → Stage-3**(이 구간은 잔여 1,184 노드시간 대비 이득이 정당화되지 않음)<br>ⓒ **< −1pp** → **즉시 중단**(퇴행) | **높음** |
...
남은 실제 제약은 달력 시간이며, **판정 시점(step 1200)까지는 순수 실행 약 1.9일**이라 그 안에 들어온다.
그리고 완만한 상승이 참일 경우의 기대 이득(+8~16pp)은 포기하기에 큰 값이다.
...
R=$W/checkpoints/grpo_expanded_gdpo/v0-20260731-094532
...
> ⚠️ **step 1200 도달 전에 스냅샷을 떠 둘 것.** `save_total_limit=5` 라 checkpoint-1200 은 step 1450 쯤 삭제된다
> (step 400 이 이미 그렇게 사라졌고, `_mideval_snap_step400` 덕분에 기준점이 살아 있다).
