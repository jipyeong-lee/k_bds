...
**최초 작성 2026-08-02 20:13 (step 650) · 최종 갱신 2026-08-04 09:20 (step 1,038)**
...
**결론부터: 인프라는 무결하고, 계획의 "1 epoch" 전제가 4배 틀렸으며, §7-1 의 재측정으로 측정 분해능 문제는 해결됐다.**

08-03 판(아래 §6·§6-c)은 "더 학습하면 오르는가"에 답할 수단이 없다는 데서 멈췄다 — 홀드아웃 n=300 은 검출 하한 8.0pp,
학습 로그는 24,000 건으로도 기울기 95% 구간이 ±14pp 였다. **§6-d 에서 홀드아웃 전량 1,772 + 짝지음(McNemar)으로
하한을 1.7~2.3pp 까지 내려 다섯 지점(init/400/500/600/700)을 같은 문항 위에서 다시 쟀고, 답이 셋으로 갈렸다:**
**RL 순효과는 확정**(init→400 +5.70pp, p<0.0001), **step 400 이후는 여전히 미검출이나 이제 상한이 붙으며**
(400→700 +1.35pp, 95% [−0.6, +3.3]pp), **의료는 네 지점 전부 무변화**다. 최종 판정은 §7-2b 대로 step 1200 에서 한다.
...
|---|---|---|
| 진행 | **1,038 / 2,337 step (44.4%)** · 벽시계 ~296 s/it | 정상 |
...
| └ 400→700 (300 step) | +1.35pp · 95% [−0.6, +3.3]pp | ⚠️ 미검출 — 큰 상승은 없으나 +3pp 는 미배제 |
| └ **의료(pmcvqa)** | init 대비 +0.25 / −0.75 / −0.25 / **+1.25**pp | 🚨 **네 지점 전부 미검출**(n=400, ±3.5pp) |
