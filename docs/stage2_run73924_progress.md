...
> ms-swift 구현(`swift/rlhf_trainers/grpo_trainer.py:556`)을 직접 판독해 확인했다.
>
> **(1) 산식 확정 · 위 실측값 역검증 성공.** `gdpo` = 보상함수별 그룹 z-score(**Bessel 보정 ddof=1**,
> `+1e-8`) → 가중합 → **배치 전역 whitening**. 위 step 903 그룹에 적용하면 `[+.10,+.10,+.10,−.30]`
> 이고, batch_std≈0.86 으로 나누면 위에 적힌 **실측 `[+0.12,+0.12,+0.12,−0.35]` 와 정확히 일치**한다.
>
> **(2) `scale_rewards=none` 이어도 똑같이 일어났다.** 그룹 중심화만으로 전원 동일 보상은 `r−r̄=0` 이므로,
> **형식 z-score = 0 은 GRPO 계열 전체의 성질**이다. 같은 그룹을 `none` 으로 계산하면
> `[+0.05, +0.05, +0.05, −0.15]` — **부호도 구조도 동일**하고 크기만 상대적으로 15% 작다.
> 즉 "형식을 어긴 짧은 출력 3개가 양수 advantage 를 받는다"는 병리는 **레시피 선택과 무관하게 발생했다.**
>
> **(3) `gdpo` 가 실제로 위험한 곳은 따로 있다** — `SoftOverlong` 처럼 **연속값** 보상이 그룹 안에서
> 거의 균일할 때다. 절단이 −1.0(완전 절단)이면 gdpo 기여는 `none` 대비 **1.15배**뿐이지만,
> 한계 초과(−0.02)면 σ 가 작아져 **57.7배**가 된다. 위 step 903 그룹은 −1.0 이었으므로 **1.15배 경우**다.
>
> ⇒ ④ 의 메커니즘 서술과 실측 표는 **그대로 유효**하다. 다만 그 책임을 `scale_rewards=gdpo` 로
> 돌리면 안 된다. 상세 = [`stage2_run73924_postmortem.md`](stage2_run73924_postmortem.md) §4-2(a).
