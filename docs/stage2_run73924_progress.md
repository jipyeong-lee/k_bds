...
홀드아웃 추세(§6)는 init → step 400 에서 오른 뒤 400·500·600 이 평평하다 — 이득이 step 400 에 이미 실현됐고
남은 1,682 step(≈1,200 노드시간)을 계속할 근거가 없다. 발표용 자립 요약은 [`stage2_overview_for_slides.md`](stage2_overview_for_slides.md).**
...
| **홀드아웃 추세 (§6)** | init 0.4533 → **400 0.4900 → 500 0.4800 → 600 0.4867** | 🚨 **step 400 포화** |
| └ 400→600 (200 step) | −0.33pp · 143 노드시간 소모 | 🚨 이득 없음 |
| └ 의료(pmcvqa) | 0.57 → 0.56 → 0.57 → 0.53 | ⚠️ 전 구간 상승 없음 |
...
**단조 증가가 아니다.** 0.4533 → 0.4900 → 0.4800 → 0.4867 — 400 이후 세 점의 폭이 **1.0pp** 로,
n=300 의 오차(±5.7pp)보다 훨씬 작아 서로 구분되지 않는다. 즉 **step 400 에서 평평해졌다.**

- init → 400: **+3.67pp** (이득이 여기서 다 나온다)
- **400 → 600: −0.33pp** — 200 step · **143 노드시간**을 쓰고 이득 0
- init → 400~600 평균: +3.23pp, p=0.428 (n=300 에선 여전히 미확정)
- 소스별: **math(mmk12) 0.40 → 0.48~0.50 이 가장 뚜렷**, deepvision 은 노이즈,
  **의료(pmcvqa)는 0.57 → 0.56 → 0.57 → 0.53 으로 상승 구간이 아예 없다**

> 판단에 필요한 것은 "이득이 있나"(미확정)가 아니라 **"더 학습하면 늘어나나"** 이고, 그 답은 명확히 아니오다.
> 잔여 1,682 step 은 같은 궤적이면 약 1,200 노드시간을 쓰고 이득이 없다.
...
| # | 조치 | 근거 | 우선도 |
|---|---|---|---|
| 1 | **Stage-2 조기중단 → Stage-3 착수** | §6 — 이득이 step 400 에 실현되고 이후 평평. 잔여 1,682 step ≈ 1,200 노드시간의 근거 없음. Stage-3 는 계획서 핵심 산출물이자 **의료를 올리도록 설계된 유일한 단계** | **높음** |
| 1b | Stage-3 init 체크포인트 선정 | 400/500/600 이 통계적으로 동일 → 최신 것 사용, 또는 상위 후보만 n 을 올려 재측정 | 높음 |
...
**판단: 조기중단이 근거에 부합한다.** 홀드아웃이 step 400 에서 평평해졌고(400~600 폭 1.0pp, 200 step 에 −0.33pp),
학습측 reward 정지·길이 인플레이션과 방향이 일치하며, 구 데이터셋의 step 600 포화 선례와도 맞는다.
무엇보다 **목표 도메인인 의료(pmcvqa)는 전 구간 상승이 없다** — Stage-2 가 의료를 올리지 못한다는 것이 실측됐고,
그것을 올리도록 설계된 단계는 미시작인 Stage-3 다.

유보: n=300 이라 8pp 미만의 완만한 상승은 배제하지 못한다. 다만 그 크기의 이득에 1,200 노드시간을 쓸 근거는 없다.
예산 자체는 부족하지 않다(끝까지 다 해도 5,000 의 약 46%) — 제약은 예산이 아니라 **달력 시간 6일**과 그 헛수고 위험이다.
